...
const LIGHT = {
  ...
  muted:"#888", border:"#E8E2D9", accent:"#2D3B8E", accentLight:"#EEF0FF",
  warm:"#C1614F", warmLight:"#F9F0EE",
  input:"#FFFFFF", inputBorder:"#E0D8CF", tag:"#EEF0FF", tagText:"#2D3B8E",
  hover:"#F5F7FF", navActive:"#2D3B8E", navActiveTxt:"#FFFFFF",
}
const DARK = {
  bg:"#0E0E0E", card:"#1A1A1A", sidebar:"#111111", text:"#F0F0F0",
  muted:"#666", border:"#2C2C2C", accent:"#5B72D4", accentLight:"#1A1D35",
  warm:"#E07060", warmLight:"#2A1815",
  input:"#222222", inputBorder:"#333333", tag:"#1A1D35", tagText:"#8899EE",
  hover:"#1A1A2A", navActive:"#5B72D4", navActiveTxt:"#FFFFFF",
}
...
const I = {
  ...
  announce: "M22 12h-4l-3 9L9 3l-3 9H2",
...
function Sidebar({ profile, currentView, setCurrentView, onLogout, T }) {
  ...
  return (
    <div style={{
      ...
      {/* User card */}
      <div style={{ margin:"0 12px 20px", background:T.accentLight, borderRadius:10,
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ fontSize:11, color:T.muted }}>Teacher</div>
        </div>
...
function DashboardView({ profile, activity, announcements, T }) {
  ...
  const typeCol = { Workshop:T.warm, Deadline:"#E05C5C", Meeting:"#4A90D9", Other:T.muted }
...
function FeedView({ profile, uid, T }) {
  ...
  const deletePost = async (postId, authorId) => {
    ...
    try { await updateDoc(doc(db,"posts",postId), { deleted:true }) } catch(e) { console.error(e) }
  }
 ...
 const filtered = posts.filter(p => {
      ...
      <div style={{ background:T.card, borderRadius:14, padding:20, border:`1px solid ${T.border}`, marginBottom:20 }}>
        <div style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
          ...
          <textarea value={text} onChange={e => setText(e.target.value)}
            placeholder="Share an update, insight or resource with your students..."
            rows={3}
            style={{ flex:1, border:`1px solid ${T.inputBorder}`, borderRadius:10, padding:"10px 12px",
              fontSize:14, resize:"none", fontFamily:"inherit", background:T.input, color:T.text, outline:"none", boxSizing:"border-box" }} />
        </div>
        <div style={{ display:"flex", justifyContent:"flex-end", marginTop:12 }}>
          <button onClick={handlePost} disabled={posting||!text.trim()} style={{
            background:text.trim()?T.accent:"#aaa", color:"#fff", border:"none", borderRadius:8,
            padding:"8px 20px", cursor:text.trim()?"pointer":"not-allowed",
  ...
  />
      ...
      <div style={{ display:"flex", gap:8, marginBottom:20, alignItems:"center", flexWrap:"wrap" }}>
        {[["all","All Posts"],["post","Posts"],["announcement","Announcements"]].map(([val,label]) => (
      ...
      {/* Posts */}
      {filtered.length===0 && <p style={{ color:T.muted, fontSize:14 }}>No posts yet.</p>}
      {filtered.map(post => {
        ...
        return (
          <div key={post.id} style={{ background:T.card, borderRadius:14, padding:20, border:`1px solid ${T.border}`, marginBottom:16 }}>
            {isAnn && (
              <span style={{ fontSize:10, fontWeight:700, color:T.warm, textTransform:"uppercase", background:T.warmLight, padding:"2px 8px", borderRadius:4, display:"inline-block", marginBottom:10 }}>📢 Announcement</span>
            )}
            <div style={{ display:"flex", gap:12, alignItems:"flex-start", marginBottom:12 }}>
              <div style={{ width:38, height:38, borderRadius:"50%", background:post.authorRole==="teacher"?T.accent:T.warmLight, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0 }}>
              ...
              {isOwner && (
                <button onClick={() => deletePost(post.id, post.authorId)} title="Delete post" style={{ background:"none", border:"none", cursor:"pointer", color:T.muted, padding:4 }}>
                  <Ico d={I.trash} size={15} color={T.muted} />
                </button>
              )}
            </div>
            ...
            <div style={{ display:"flex", gap:16, paddingTop:12, borderTop:`1px solid ${T.border}` }}>
              <button onClick={() => toggleLike(post.id, post.likes)} style={{
                ...
                cursor:"pointer", color:liked?T.warm:T.muted, fontSize:13,
              }}>
                <Ico d={I.heart} size={15} color={liked?T.warm:T.muted} />
...
// ── Match / Students view (Teacher sees all students) ─────────
...
const handleAiSearch = async () => {
  ...
  const filtered = users.filter(u => {
    ...
    return (
      ...
      u.semester?.toLowerCase().includes(q) ||
  ...
  const groups   = filter==="all"
    ? [["Students",students],["Other Teachers",teachers]]
    : filter==="student"
      ? [["Students",filtered]]
      : [["Teachers",filtered]]
      ...
      <div style={{ background:T.card, borderRadius:14, padding:20, border:`1px solid ${T.border}` }}>
        <div style={{ display:"flex", gap:14, alignItems:"flex-start", marginBottom:12 }}>
          <div style={{ width:48, height:48, borderRadius:12,
            background:isTeacher?T.accentLight:T.warmLight,
            display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, flexShrink:0 }}>
            {isTeacher?"🧑‍🏫":"🎓"}
          </div>
          <div style={{ flex:1 }}>
            <p style={{ margin:"0 0 3px", fontWeight:700, fontSize:14, color:T.text }}>{u.name||"Unknown"}</p>
            <span style={{ fontSize:11, fontWeight:600,
              color:isTeacher?T.accent:T.warm,
              background:isTeacher?T.accentLight:T.warmLight,
        ...
        {(u.department||u.semester) && (
          <p style={{ margin:"0 0 6px", fontSize:12, color:T.muted, display:"flex", alignItems:"center", gap:4 }}>
            <Ico d={I.book} size={12} color={T.muted} />
            {[u.department, u.semester].filter(Boolean).join(" · ")}
          </p>
        )}
        {u.bio && (
  ...
  return (
    <div>
      ...
      <p style={{ color:T.muted, fontSize:14, margin:"0 0 24px" }}>Find and connect with students and colleagues</p>
{/* AI Search */}
<div style={{
...
{aiResults !== null && (
  <div style={{ marginBottom:24 }}>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:16 }}>
      ...
      <div style={{ display:"flex", gap:12, marginBottom:24, alignItems:"center", flexWrap:"wrap" }}>
        <div style={{ flex:1, minWidth:200, position:"relative" }}>
          <div style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", pointerEvents:"none" }}>
            <Ico d={I.search} size={16} color={T.muted} />
          </div>
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Search by name, department, skill, or interest..."
            style={{ width:"100%", border:`1px solid ${T.inputBorder}`, borderRadius:10,
              padding:"10px 12px 10px 36px", fontSize:14, background:T.input,
              color:T.text, boxSizing:"border-box", outline:"none" }} />
        </div>
        <div style={{ display:"flex", gap:6 }}>
          {[["all","Everyone"],["student","Students"],["teacher","Teachers"]].map(([val,label]) => (
            <button key={val} onClick={() => setFilter(val)} style={{
              padding:"8px 14px", borderRadius:8, border:`1px solid ${filter===val?T.accent:T.border}`,
              background:filter===val?T.accent:"transparent",
              color:filter===val?"#fff":T.muted, fontSize:12,
              cursor:"pointer", fontWeight:filter===val?600:400, whiteSpace:"nowrap",
            }}>{label}</button>
          ))}
        </div>
      </div>
      ...
      {groups.map(([groupLabel, groupUsers]) => (
        groupUsers.length===0 ? null : (
          <div key={groupLabel} style={{ marginBottom:32 }}>
            <h2 style={{ fontSize:14, fontWeight:700, color:T.muted, textTransform:"uppercase", letterSpacing:1, margin:"0 0 16px" }}>
              {groupLabel} ({groupUsers.length})
            </h2>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:16 }}>
              {groupUsers.map(u => <UserCard key={u.id} u={u} />)}
            </div>
          </div>
        )
      ))}
...
// ── Announce (Teacher only) ───────────────────────────────────
function AnnounceView({ profile, uid, T }) {
  const [form, setForm]       = useState({ title:"", body:"", type:"Workshop", date:"", time:"" })
  const [posting, setPosting] = useState(false)
  const [success, setSuccess] = useState(false)
  const [error, setError]     = useState("")
  const [myAnn, setMyAnn]     = useState([])
  ...
  const handlePost = async () => {
    if (!form.title.trim()||!form.body.trim()) { setError("Title and description are required."); return }
    setError("")
    setPosting(true)
    try {
      // Save to announcements collection (shows in dashboard upcoming)
      ...
      // Also publish to feed so everyone sees it
      await addDoc(collection(db,"posts"), {
      ...
      setSuccess(true)
      setForm({ title:"", body:"", type:"Workshop", date:"", time:"" })
  ...
  return (
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:28, alignItems:"start" }}>
      {/* Compose */}
      <div>
        <h1 style={{ fontSize:24, fontWeight:700, margin:"0 0 6px", color:T.text }}>Post Announcement</h1>
        <p style={{ color:T.muted, fontSize:13, margin:"0 0 22px" }}>Visible to all students on the platform and in their Feed.</p>

        <div style={{ background:T.card, borderRadius:14, padding:24, border:`1px solid ${T.border}` }}>
          {/* Title */}
          <div style={{ marginBottom:14 }}>
            <label style={{ fontSize:12, color:T.muted, display:"block", marginBottom:4 }}>Announcement Title *</label>
            <input value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))}
              placeholder="e.g. Extra class on Saturday"
              style={{ width:"100%", border:`1px solid ${T.inputBorder}`, borderRadius:8, padding:"10px 12px", fontSize:14, background:T.input, color:T.text, boxSizing:"border-box" }} />
          </div>
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <select value={form.type} onChange={e=>setForm(f=>({...f,type:e.target.value}))}
              style={{ width:"100%", border:`1px solid ${T.inputBorder}`, borderRadius:8, padding:"10px 12px", fontSize:14, background:T.input, color:T.text, boxSizing:"border-box" }}>
              {["Workshop","Deadline","Meeting","Other"].map(o=><option key={o}>{o}</option>)}
            </select>
          </div>

          {/* Body */}
          <div style={{ marginBottom:14 }}>
            <label style={{ fontSize:12, color:T.muted, display:"block", marginBottom:4 }}>Description *</label>
            ...
            {[["Date","date","date"],["Time","time","time"]].map(([label,key,type])=>(
              <div key={key}>
                ...
                <input type={type} value={form[key]} onChange={e=>setForm(f=>({...f,[key]:e.target.value}))}
                  style={{ width:"100%", border:`1px solid ${T.inputBorder}`, borderRadius:8, padding:"9px 12px", fontSize:13, background:T.input, color:T.text, boxSizing:"border-box" }} />
              </div>
            ))}
          </div>

          {error && <div style={{ background:"#FEE", color:"#C00", borderRadius:8, padding:"9px 12px", marginBottom:12, fontSize:13 }}>⚠️ {error}</div>}
          {success && <div style={{ background:"#E8F5E9", color:"#2E7D32", borderRadius:8, padding:"9px 12px", marginBottom:12, fontSize:13, fontWeight:500 }}>✅ Announcement posted! Visible to all students.</div>}

          <button onClick={handlePost} disabled={posting} style={{
            width:"100%", background:T.accent, color:"#fff", border:"none", borderRadius:10,
            padding:"13px", cursor:posting?"not-allowed":"pointer", fontSize:14, fontWeight:600,
            display:"flex", alignItems:"center", justifyContent:"center", gap:8, opacity:posting?0.7:1,
          }}>
            <Ico d={I.plus} size={16} color="#fff" />
      ...
      {/* My Announcements */}
      <div>
        <h2 style={{ fontSize:18, fontWeight:700, margin:"0 0 16px", color:T.text }}>My Announcements</h2>
        {loadingAnn && <p style={{ color:T.muted, fontSize:14 }}>Loading...</p>}
        {!loadingAnn && myAnn.length===0 && (
          <div style={{ background:T.card, borderRadius:12, padding:24, border:`1px solid ${T.border}`, textAlign:"center" }}>
            <div style={{ fontSize:32, marginBottom:8 }}>📭</div>
            <p style={{ color:T.muted, fontSize:14, margin:0 }}>No announcements posted yet.</p>
          </div>
        )}
        {myAnn.map((a,i) => {
          const col = typeCol[a.type]||T.muted
          return (
            <div key={i} style={{ background:T.card, borderRadius:12, padding:18, border:`1px solid ${T.border}`, marginBottom:12 }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
                <span style={{ fontSize:10, fontWeight:700, color:col, textTransform:"uppercase", background:`${col}20`, padding:"2px 7px", borderRadius:4 }}>{a.type}</span>
                <span style={{ fontSize:11, color:T.muted }}>{timeAgo(a.postedAt)}</span>
              </div>
              <p style={{ margin:"0 0 4px", fontSize:14, fontWeight:600, color:T.text }}>{a.title}</p>
              <p style={{ margin:"0 0 8px", fontSize:13, color:T.muted, lineHeight:1.5, overflow:"hidden", display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical" }}>{a.body}</p>
              {a.date && <p style={{ margin:0, fontSize:12, color:T.muted }}>📅 {a.date}{a.time?` at ${a.time}`:""}</p>}
            </div>
          )
        })}
      </div>
...
function ProfileView({ profile, uid, onProfileUpdate, T }) {
  ...
  const [form, setForm]       = useState({ name:"", bio:"", location:"", department:"", designation:"", subject:"", interests:"", skills:[] })
  ...
  return (
    <div>
      ...
      <div style={{ background:T.accent, borderRadius:14, padding:"28px 32px", display:"flex", alignItems:"center", gap:24, marginBottom:24, position:"relative" }}>
        <div style={{ width:78, height:78, borderRadius:14, background:"rgba(255,255,255,0.15)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:38, flexShrink:0 }}>🧑‍🏫</div>
        <div style={{ flex:1 }}>
          ...
          <p style={{ color:"rgba(255,255,255,0.7)", margin:"0 0 8px", fontSize:13 }}>
            {profile?.designation||"Faculty"}{profile?.subject?` · ${profile.subject}`:""}
          </p>
        ...
        <button onClick={()=>setEditing(e=>!e)} style={{ position:"absolute", top:20, right:20, background:"rgba(255,255,255,0.15)", border:"1px solid rgba(255,255,255,0.3)", color:"#fff", borderRadius:8, padding:"8px 16px", cursor:"pointer", display:"flex", alignItems:"center", gap:6, fontSize:13, fontWeight:500 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
          ...
          <div style={{ background:T.card, borderRadius:12, padding:24, border:`1px solid ${T.border}` }}>
            ...
            {editing
              ? <textarea value={form.bio} rows={3} onChange={e=>setForm(f=>({...f,bio:e.target.value}))}
                  placeholder="Describe your teaching philosophy and research interests..."
                  style={{ width:"100%", border:`1px solid ${T.inputBorder}`, borderRadius:8, padding:"10px 12px", fontSize:14, resize:"vertical", fontFamily:"inherit", background:T.input, color:T.text, boxSizing:"border-box" }} />
              : <p style={{ margin:0, color:profile?.bio?T.text:T.muted, fontSize:14, lineHeight:1.6 }}>{profile?.bio||"Add a bio to introduce yourself to students."}</p>
            }
          </div>
          ...
          <div style={{ background:T.card, borderRadius:12, padding:24, border:`1px solid ${T.border}` }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
              <h3 style={{ margin:0, fontSize:15, fontWeight:600, color:T.text, display:"flex", alignItems:"center", gap:8 }}><Ico d={I.code} size={15} color={T.accent} />Expertise</h3>
            ...
            {skills.length===0 && <p style={{ color:T.muted, fontSize:13 }}>No expertise areas added yet.</p>}
            {skills.map((sk,i) => (
              <div key={i} style={{ marginBottom:14 }}>
                {editing
                  ? <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:5 }}>
                      <input value={sk.name} onChange={e=>updateSkill(i,"name",e.target.value)} placeholder="Area of expertise"
                        style={{ flex:1, border:`1px solid ${T.inputBorder}`, borderRadius:6, padding:"6px 10px", fontSize:13, background:T.input, color:T.text }} />
                      <input type="number" value={sk.level} min={0} max={100}
                        onChange={e=>updateSkill(i,"level",Number(e.target.value))}
                        style={{ width:58, border:`1px solid ${T.inputBorder}`, borderRadius:6, padding:"6px 8px", fontSize:13, textAlign:"center", background:T.input, color:T.text }} />
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          <div style={{ background:T.card, borderRadius:12, padding:20, border:`1px solid ${T.border}` }}>
            ...
            {editing
              ? [
                  ...
                  ["Designation","designation","e.g. Associate Professor"],
                  ["Department","department","e.g. CSE"],
                  ["Subject","subject","e.g. Data Structures"],
                  ["Location","location","e.g. Jaipur"],
                ].map(([label,key,ph])=>(
          ...
          <div style={{ background:T.card, borderRadius:12, padding:20, border:`1px solid ${T.border}` }}>
            ...
            {editing
              ? <input value={form.interests} onChange={e=>setForm(f=>({...f,interests:e.target.value}))}
                  placeholder="ML, Cloud, Research (comma separated)"
                  style={{ width:"100%", border:`1px solid ${T.inputBorder}`, borderRadius:6, padding:"7px 10px", fontSize:13, background:T.input, color:T.text, boxSizing:"border-box" }} />
          ...
          <div style={{ background:T.accent, borderRadius:12, padding:20 }}>
            ...
            {(profile?.achievements||[]).length===0
              ? <p style={{ color:"rgba(255,255,255,0.6)", fontSize:13, margin:0 }}>Achievements will appear here.</p>
              : (profile.achievements).map((a,i)=><p key={i} style={{ margin:"0 0 5px", color:"rgba(255,255,255,0.85)", fontSize:13 }}>🏆 {a}</p>)
            }
          </div>

          {editing && (
            <button onClick={save} disabled={saving} style={{ background:T.warm, color:"#fff", border:"none", borderRadius:10, padding:"12px", cursor:saving?"not-allowed":"pointer", fontSize:14, fontWeight:600, opacity:saving?0.7:1 }}>
...
function SettingsView({ profile, uid, dark, setDark, onProfileUpdate, T }) {
  ...
  const row = (label, children) => (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"17px 0", borderBottom:`1px solid ${T.border}` }}>
  ...
  return (
    <div style={{ maxWidth:540 }}>
      ...
      <div style={{ background:T.card, borderRadius:14, padding:"0 24px", border:`1px solid ${T.border}`, marginBottom:20 }}>
        ...
        {row("Role",        <span style={{ fontSize:13, fontWeight:600, color:T.accent, background:T.accentLight, padding:"3px 10px", borderRadius:10 }}>Teacher</span>)}
        {row("Designation", <span style={{ fontSize:13, color:T.muted }}>{profile?.designation||"Not set"}</span>)}
      </div>
      ...
      <div style={{ background:T.card, borderRadius:14, padding:"0 24px", border:`1px solid ${T.border}`, marginBottom:20 }}>
        ...
        {row("Current Theme", <span style={{ fontSize:13, color:T.muted }}>{dark?"Dark":"Light"}</span>)}
      </div>
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (fu) => {
      ...
      try {
        ...
        if (data.role && data.role!=="teacher") { navigate("/dashboard/student"); return }
        setUser(fu)
